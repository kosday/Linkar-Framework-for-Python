...
class MvOperations:
	...
	"""
	@staticmethod
	def LkChange(input_string, strOld, strNew, occurrence=0, start=1):
		...
		index = 0
		while start > 1:
			index = input_string.find(strOld, index)
			if index == -1:
				break
			index += len(strOld)
			start -= 1
		...
		if not occurrence or occurrence < 1:
			return part1 + part2.replace(strOld, strNew)
		else:
			return part1 + part2.replace(strOld, strNew, occurrence)
	...
	def LkReplace(input_string, newVal, field, value=0, subvalue=0):
		...
		if value > 0:
			...
			for index in range(value):
				createdstr += cstr
			####input_string += createdstr
			input_string = input_string[:i] + createdstr + input_string[i:]
			i += value
